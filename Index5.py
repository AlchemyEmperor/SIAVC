...
def main():
    ...
    parser.add_argument('--num-labeled', type=int, default=134,   #SI9 134   SI7 108
    ...
    parser.add_argument('--eval-step', default=34, type=int,
                        help='number of eval steps to run 768')
    parser.add_argument('--start-epoch', default=0, type=int,
                        help='manual epoch number (useful on restarts)')
    parser.add_argument('--batch-size', default=4, type=int,
                        help='train batchsize')
    # parser.add_argument('--loader-size', default=16, type=int,
    ...
    parser.add_argument(
        '--list-threshold', default=0.90, type=float, help='threshold for synthetic queue threshold')
   ...
   #----------------------------------------------------------------------------
    def create_model(args):
        # import all_model.generate_model as generate_model  # main(cnn_name, model_depth, n_classes, in_channels, sample_size)
        # model = generate_model.main(cnn_name=args.arch,
        #                             model_depth=18,
        #                             n_classes=args.num_classes,
        #                             in_channels=3,
        #                             sample_size=args.batch_size + args.mu * args.batch_size)
        ...
        # weights = torch.load('./Pretrained/model_best_lhx.pth', map_location=args.device)
    ...
    # For SI9 wothout Normal
    datapath = ['/data/lqh/Key/FireMatchV2/dataset/SI9t/Train']
    test_datapath = ['/data/lqh/Key/FireMatchV2/dataset/SI9t/Test']
    weak_augmenta_datapath = ['/data/lqh/Key/FireMatchV2/dataset/SI9t/Train']
    strong_augmenta_datapath = ['/data/lqh/Key/FireMatchV2/dataset/SI9t/Train']
    
    # For Fire Detection
    #datapath = ['/data/lqh/Key/VideoMatch/dataset/Fire_and_Smoke/Train']
    #test_datapath = ['/data/lqh/Key/VideoMatch/dataset/Fire_and_Smoke/Test']
    #weak_augmenta_datapath = ['/data/lqh/Key/VideoMatch/dataset/Fire_and_Smoke/Train']
    #strong_augmenta_datapath = ['/data/lqh/Key/VideoMatch/dataset/Fire_and_Smoke/Train']
...
def train(args,labeled_train_dataloader, unlabeled_train_dataloader, strong_dataloader,weak_dataloader,test_loader,
    ...
    model.train()
    loss_data = pd.DataFrame(columns=['Epoch', 'Lx', 'Lu', 'Lf', 'La', 'Loss'])
    #class
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            try:
                inputs_u_x, _, index_u = unlabeled_iter.__next__()
            except:
                unlabeled_iter = iter(unlabeled_train_dataloader)
                inputs_u_x, _, index_u = unlabeled_iter.__next__()
            ...
            mask = torch.ge(max_probs, threshold).float()  # FreeMatch  




            in_lx2, x1, x2 = convert2(inputs_x)
            in_ux2, _, _ = convert2(inputs_u_x)

            label_onehot = torch.nn.functional.one_hot(targets_x.to(torch.int64),
            ...
            mixup_video, mixup_label, mix_indice = mixup(
                in_lx2,  # (4,96,160,160)*3
                label_onehot,  # (4,101) [1.,0.]
                in_ux2,  # (12,96,160,160)
                pseudo_label,# (12,101) [0.5643.0.4357]
                inputs_x.shape[0],
                args
            )
            ...
            ada_total_loss = (ada_reg_loss + ada_adv_loss) * 0.1
        ...
        loss_data = loss_data.append({'Epoch': epoch, 'Lx': a/args.eval_step, 'Lu': b/args.eval_step,
                                      'Lf': c/args.eval_step, 'La': d/args.eval_step, 'Loss': e/args.eval_step}, ignore_index=True)
        ...
        if args.local_rank in [-1, 0]:
            ...
            test_loss, test_acc_a, test_acc5_a, class_acc_list = test(args, test_loader, test_model, False)
            ...
            for i in range(len(class_acc_list)):
                ...
                print("Class: {:<20} acc={:<7.2f}".format(class_dict[int(i)], class_acc_list[i] * 100))
            ...
            if is_best:
                ...
                best_acc5_a = test_acc5_a
                # best_class_acc = class_acc_list
                # avg_acc = 0.0
                # for i in range(len(best_class_acc)):
                #     avg_acc += (best_class_acc[i] * 100) / args.num_classes
                #     print("Class: {:<20} acc={:<7.2f}".format(class_dict[int(i)], best_class_acc[i] * 100))
                # print("Average acc=", avg_acc)
            ...
            # for i, acc in enumerate(best_class_acc):
            #     logger.info("Class {} accuracy: {:.2f}".format(i, acc * 100))
...
def test(args, test_loader, model, Test):
    ...
    if Test :
        weights = torch.load('./Pretrained/SI9_ViT_83.44%.pth.tar', map_location=args.device)
    ...
    with torch.no_grad():
        ...
        if t_SNE:  # 3D
            sizes = np.zeros_like(labels) + 100
            features = np.concatenate(features)
            labels = np.concatenate(labels)
            tsne = TSNE(n_components=3, perplexity=30.0, learning_rate=200.0, n_iter=1000)
            features_tsne = tsne.fit_transform(features)
            fig = plt.figure()
            ax = fig.add_subplot(111, projection='3d')

            # Set colors for the classes
            colors = ['black', 'gray']  # Set the desired colors for each class

            # Plot each class separately with its own color
            unique_labels = np.unique(labels)
            for label in unique_labels:
                class_indices = labels == label
                color = colors[label]  # Assign the corresponding color for the class
                ax.scatter(features_tsne[class_indices, 0], features_tsne[class_indices, 1],
                           features_tsne[class_indices, 2],
                           color=color, s=sizes[class_indices])
        ...
        if use_UMAP:  # 2D
            ...
            colors = ['black', 'gray', 'red', 'blue', 'green', 'purple', 'orange', 'cyan', 'magenta']
            ...
            for label in unique_labels:
                class_indices = labels == label
                color = colors[label]
                ax.scatter(features_umap[class_indices, 0], features_umap[class_indices, 1],
                           color=color, s=sizes[class_indices])
        ...
        #     features_tsne = tsne.fit_transform(features)
        #     plt.scatter(features_tsne[:, 0], features_tsne[:, 1], c=labels, s=sizes, cmap=cmap) 
        #     plt.show()
    ...
    return losses.avg, top1_a.avg, top5_a.avg, result_list
